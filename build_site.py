...
var fname=decodeURIComponent((p.split("/").pop()||""));
["open","open2"].forEach(function(id){document.getElementById(id).href=src;});
["dl","dl2"].forEach(function(id){var a=document.getElementById(id);a.href=src;a.setAttribute("download",fname);});
var fr=document.getElementById("fr"),load=document.getElementById("load");
var isMobile=/Android|iPhone|iPad|iPod|Mobile|Silk/i.test(navigator.userAgent)||window.innerWidth<700;
if(!src){document.getElementById("lt").textContent="未指定文件";}
else if(isMobile){
  // 移动端浏览器多不支持 iframe 内嵌 PDF，直接跳转交给原生阅读器
  fr.style.display="none";
  document.getElementById("lt").textContent="正在打开 PDF…";
  document.getElementById("slow").style.display="block";
  setTimeout(function(){location.replace(src);},400);
}else{
  fr.onload=function(){load.classList.add("hide");};
  fr.src=src;
  setTimeout(function(){if(!load.classList.contains("hide"))document.getElementById("slow").style.display="block";},6000);
}
</script>
</body>
</html>'''
